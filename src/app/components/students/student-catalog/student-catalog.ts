...
@Component({
  ...
  imports: [CommonModule, FormsModule, ActivityCard, SearchForm],
...
export class StudentCatalog implements OnInit {
  ...
  getAvailableCapacity(activity: Activity): number {
    const confirmedEnrollments = this.allEnrollments.filter(e =>
  ...
  enrollInActivity(activityId: number) {
    const existingEnrollment = this.allEnrollments.find(e =>
    ...
    if (existingEnrollment) {
        alert(`Ya estás inscrito con estado: ${existingEnrollment.status}. Revisa Mis Inscripciones.`);
        return;
    }
    ...
    this.enrollmentService.create(newEnrollment).subscribe({
      next: () => {
        alert("¡Solicitud de inscripción enviada! Estado: PENDIENTE. Revisa Mis Inscripciones.");
        this.loadInitialData();
      },
      error: (err) => {
